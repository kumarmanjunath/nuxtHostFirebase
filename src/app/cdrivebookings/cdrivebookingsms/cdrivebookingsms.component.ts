...
@Component({
  selector: 'app-cdrivebookingsms',
  templateUrl: './cdrivebookingsms.component.html',
  styleUrls: ['./cdrivebookingsms.component.scss']
})
export class CdrivebookingComponentsms implements OnInit {
  endpoint = 'https://asia-east2-hype-54c03.cloudfunctions.net/sendcustomersms';
  constructor(private http: HttpClient, public service: CdrivebookingService,private afStorage: AngularFireStorage,private formBuilder: FormBuilder,public dialogRef: MatDialogRef<CdrivebookingComponentsms>, private firestore: AngularFirestore, private toastr: ToastrService) { }
    ...
    onSubmit() {
      ...
      this.firestore.collection('driver_transaction').add(docData).then(docRefs => {
        //alert(this.service.smsmid)
              this.firestore.collection("cdrive_booking")
              ...
              this.http.get(this.endpoint+"?cusname="+cusname+"&drivername="+drivername+"&driverphone="+driverphone+"&driverreg="+driverreg+"&secretcode="+secretcode+"&cusphone="+this.service.smscusphone+"&smspackage="+smspackage+"&smscarname="+smscarname).subscribe((res : any[])=>{ */
                ...
                var mystring=this.endpoint+"?cusname="+cusname+"&drivername="+drivername+"&driverphone="+driverphone+"&driverreg="+driverreg+"&secretcode="+secretcode+"&cusphone="+this.service.smscusphone+"&bookingid="+this.service.smsbookingid+"&smspackage="+smspackage+"&smscarname="+smscarname;
